...
function getStandardizedErrorMessage(error, context = '') {
    ...
    // فحص أخطاء الخادم السحابي غير المفعل
    if (errorMsg.includes('google apps script غير مفعل') ||
        ...
        return {
            type: 'disabled',
            message: error?.message || String(error),
            userMessage: 'الخادم السحابي غير مفعل.\nسيتم استخدام البيانات المحلية.',
            showRetry: false
        };
    }
    ...
    if (errorMsg.includes('scripturl') ||
        ...
        return {
            type: 'config',
            message: error?.message || String(error),
            userMessage: 'إعدادات الخادم السحابي غير مكتملة.\nيرجى التحقق من الإعدادات.',
            showRetry: false
        };
    }
...
async function safeGoogleRequest(requestData, timeout = 10000, operation = 'الطلب') {
    try {
        // التحقق من تكوين الخادم السحابي
        if (typeof AppState === 'undefined' || !AppState.backendConfig?.server?.enabled) {
            throw new Error('الخادم السحابي غير مفعل');
        }

        if (!AppState.backendConfig.server.scriptUrl ||
            AppState.backendConfig.server.scriptUrl.trim() === '') {
            throw new Error('الخادم السحابي غير مُكوَّن بشكل صحيح');
        }

        // التحقق من توفر Backend
        if (typeof Backend === 'undefined' ||
            typeof Backend.sendRequest !== 'function') {
            throw new Error('Backend غير متاح');
        }

        // إرسال الطلب مع timeout
        return await promiseWithTimeout(
            Backend.sendRequest(requestData),
            timeout,
            operation
        );
    } catch (error) {
